...
TTS は**2つある音源のうちの1つ**で、もう一方は音声ファイル（音声パック）。
どちらで鳴らすかの決定は `PronunciationService` が行う（[pronunciation.md] §2）。
本書は TTS 側の詳細だけを定める。

UI からは `PronunciationService` だけを呼び、`TtsService` を直接呼ばない。
例外は例文の読み上げで、これは常に TTS になるが、それも `speakText()` 経由で呼ぶ。
...
音声パックの有無も合わせて判定する。**「TTS が使えない」と「その語が鳴らせない」は別**で、
UI が見るのは後者（`PronunciationService.resolve()` の結果）になる。

| 状況 | UI |
|---|---|
| 英語 voice が無く、英語の音声パックも無い | リスニングモードのカード・選択肢を非表示。英語の 🔊 を非表示 |
| 英語 voice は無いが音声パックがある | リスニングモードは使える（音声ファイルで出題する）。例文の 🔊 だけ消える |
| 日本語 voice が無く、日本語の音声パックも無い | 日本語の 🔊 とフラッシュカード `speakJa` を非表示 |
| 音声パックも voice も1つも無い | 設定 > 学習の音声カードごと非表示にし、「この端末では音声を再生できません」の1行に置き換える |
| 再生が失敗した | SnackBar「読み上げに失敗しました: 理由」。自動送りは止める。**別の音源に切り替えない**（[pronunciation.md] §2.2） |
