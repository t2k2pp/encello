...
| choice | 正解 | 4 |
| choice | 不正解 | 1 |
| flashcard（確認テスト・4択） | choice と同じ | 4 / 1 |
| flashcard（確認テスト・スペル） | spell と同じ | 5 / 4 / 3 / 1 / 0 |
| flashcard（流し見） | 解答が無い | 更新しない |
| speed | 時間内に正解 | 4 |
...
- フラッシュカードは**確認テストを解いたときだけ** `word_reviews` を更新する。
  流し見しているだけの語は既習にしない（FR-26）。専用の grade は持たず、
  確かめた形式の入口（`forChoice` / `forSpell`）をそのまま通す
  （[flashcard_mode.md] §6）。
