...
| SCR-05 | フラッシュカード | 3種の自動送り ＋ ラウンドごとの確認テスト |
...
5. **下書きカード**: マイ単語の下書きが3語以上あるとき「訳が空の単語が5語あります」。
   タップで SCR-17 の下書き整備へ。0〜2語のときは出さない（[06_features/my_words.md] §3）。
...
自己評価のボタンは置かない。覚えたかどうかは、このあとの確認テストが判定する
（[06_features/flashcard_mode.md] §4）。
...
ラウンドの末尾まで送ると、そのラウンドで見せた語だけを問う画面に切り替わる。
...
├──────────────────────────────────┤
│           りんご                  │
│  [ apple ]                       │  4択は SCR-06 と同じ描画
│  [ orange ]                      │  スペルは SCR-04 と同じ入力・判定
│  [ grape ]                       │
│  [ peach ]                       │
└──────────────────────────────────┘
```

テストの体裁を別に作らない。4択なら `ChoiceQuestionView`、スペルなら
`SpellPrompt` + `EnglishKeyboard` + `VerdictBanner` を、それぞれのモードと共有する。
...
| 学習 | デイリー目標 / 1セッションの問題数 / キーボード配列 / フラッシュカードの確認テスト（形式・ラウンドの枚数）/ 正解したら自動で次へ / フラッシュカードの送り方と秒数 / 4択の出題方向 / スピードの制限時間 / **学習リマインダー**（ON・時刻・テスト通知）/ **語彙力を測る**（SCR-18 へ）/ **音声**（音源の優先順位・音声パック・合成音声の voice/速さ/高さ・試聴。[06_features/pronunciation.md] §7） |
| マスタ | **学習者管理**（SCR-22）/ 単語帳管理（SCR-12）/ **マイ単語**（SCR-17）/ **音声パック**（SCR-23）/ 実績（SCR-14）へのタイル |
| データ | **AI に単語帳を作ってもらう**（SCR-25）/ **貼り付けて取り込む**（SCR-24）/ バックアップ（書き出し・復元）/ 単語帳の CSV（書き出し・取り込み）/ サンプルデータ（投入・削除の2列）/ 未所属の単語を整理 / 使われていない音声ファイルを整理 / 学習状態をリセット（二段確認・「先にバックアップを書き出す」導線） |
...
- 4択と同じ配置。選択肢の上に**残り時間のバーと「あと N 秒」**を出す。
- 問題を出す前にカウントダウンを挟む（最初は3秒、問題と問題のあいだは1秒）。
  数え終わるまで問題は隠す（[06_features/speed_mode.md] §2.1）。
...
- スピードモードだけは `Feedback` を持たず、`Judging` から 0.4 秒後に自動で `Presenting` へ戻る。
  ...
  ただし `Presenting` に入ってすぐは問題を隠してカウントダウンを出し、数え終わってから
  `markPresented()` で反応時間の計測を始める（[06_features/speed_mode.md] §2.1）。
